...
def test_charge_model():
    record = ChargingModel(
        charge_time="0:30", charge_amount=7.8, submit_time=datetime.utcnow()
    )
...
def test_trip_with_range():
    range_model = RangeModel(
        battery_range=100, percentage=80, submit_time=datetime.utcnow()
    )
    trip = TripModel(
        miles=1.5,
        kwh=3.4,
        trip_time="0:15",
        destination="RangeTest",
        submit_time=datetime.utcnow(),
        battery_range=range_model,
    )
    assert trip.battery_range == range_model
